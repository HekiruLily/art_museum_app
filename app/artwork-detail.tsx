...
import { MusicPlayer, MiniPlayer } from '@/components/music';
...
export default function ArtworkDetailScreen() {
  ...
  const artworkId = parseInt(params.artworkId as string);
  
  const [artwork, setArtwork] = useState<any>(null);
  const [loading, setLoading] = useState(true);
  const { toggleFavorite, isFavorite } = useFavorites();
  const favorited = artwork ? isFavorite(artwork.objectID) : false;
  
  // Use shared music context
  const { loadTracks, playableTracks, pause } = useMusic();
  ...
  if (!artwork) {
    return (
      <View style={styles.loadingContainer}>
        <Text style={styles.errorText}>Không tìm thấy tác phẩm</Text>
    ...
    <View style={styles.container}>
      ...
      <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={false}>
        ...
        >
          ...
          <View style={styles.imageTapHint}>
            <Text style={styles.imageTapHintText}>
              {Platform.OS === 'web' ? '🖼️ Nhấn để xem toàn màn hình' : '🎵 Nhấn để thưởng thức'}
            </Text>
          </View>
        </TouchableOpacity>
        ...
        <View style={styles.contentCard}>
          ...
          <View style={styles.descriptionSection}>
            ...
            <Text style={styles.descriptionText}>
              {artwork.artistDisplayBio || artwork.creditLine || 'Thông tin về tác phẩm nghệ thuật này từ bộ sưu tập của Bảo tàng Metropolitan.'}
            </Text>
          </View>
          ...
          {artwork.additionalImages && artwork.additionalImages.length > 0 && (
            <View style={styles.additionalSection}>
              ...
              >
                {artwork.additionalImages.map((imageUrl: string, index: number) => (
                  <Image
                    key={index}
                    source={{ uri: imageUrl }}
      ...
      >
        <View style={styles.fullImageContainer}>
          ...
          {Platform.OS === 'web' && (
            <View style={styles.musicLoadingContainer}>
              <Text style={styles.musicLoadingText}>
                🎵 Tính năng phát nhạc chỉ khả dụng trên mobile app
              </Text>
            </View>
          )}

          {/* Mini Player */}
          {!loadingMusic && playableTracks.length > 0 && Platform.OS !== 'web' && (
            <MiniPlayer
              artwork={artwork}
